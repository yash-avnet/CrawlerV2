...
export async function saveProductsToDB(supabase: any, products: ProductInfo[]): Promise<void> {
  ...
  try {
    ...
    const mpnToIdMap = new Map(insertedProducts.map((p: any) => [p.mpn, p.id]));
